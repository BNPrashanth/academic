...
json_data = None
with open('/data/chat_dataset.json') as data_file:
...
formatted_data = data['message'].apply(
    lambda x: " ".join([stemmer.stem(i) for i in re.sub("[^a-zA-Z]", " ", x).split() if i not in words]).lower()
)

X_train, X_test, y_train, y_test = train_test_split(formatted_data, data.label, test_size=0.2)
